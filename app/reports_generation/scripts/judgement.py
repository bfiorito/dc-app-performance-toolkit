...
def judge(dataframe_baseline: pandas.DataFrame, dataframe_tested: pandas.DataFrame, measurement_by_column: str):
	judgement_results = []
...
def judge_baseline_vs_tested(baseline_result_filepath, tested_result_filepath):
	raw_dataframe_baseline = files_to_dataframe(baseline_result_filepath)
	raw_dataframe_experiment = files_to_dataframe(tested_result_filepath)
	# Map timelines to each of action appropriately
	df_baseline = group_data_by_column(raw_dataframe_baseline, columns=('label', ))
	df_tested = group_data_by_column(raw_dataframe_experiment, columns=('label', ))

	results = judge(df_baseline, df_tested, measurement_by_column='elapsed')

	for judgement_result in results:
		if not judgement_result.passed:
			# TODO: later on we are going to send somehow the log to reporting tool and print results differently
			print(f"ERROR: Tested version is bigger for action {judgement_result.action}."
			      f"{baseline_result_filepath}-{tested_result_filepath}")

	return results


def judge_results(baseline_result_dir, tested_result_dir):
	jmeter_filepaths = (os.path.join(baseline_result_dir, 'kpi.jtl'),
	                    os.path.join(tested_result_dir, 'kpi.jtl'))
	results = judge_baseline_vs_tested(*jmeter_filepaths)
	...
	save_results([results[0].head()] + results_representation, filepath=os.path.join(tested_result_dir, 'kpi_judged.csv'))

	selenium_filepaths = (os.path.join(baseline_result_dir, 'selenium.jtl'),
	                      os.path.join(tested_result_dir, 'selenium.jtl'))
	results = judge_baseline_vs_tested(*selenium_filepaths)
